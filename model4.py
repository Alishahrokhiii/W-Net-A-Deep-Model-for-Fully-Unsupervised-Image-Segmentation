...
class ConvModule(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        layers = [
            ...
            nn.InstanceNorm2d(output_dim), #disable
            nn.BatchNorm2d(output_dim),
            nn.ReLU(),
            nn.Dropout(config.drop),#disable
...
class BaseNet(nn.Module): # 1 U-net
    ...
    encoder=[64, 128, 256, 512], decoder=[1024, 512, 256], output_channels=config.k): #64 ? first darrim !?
...
class BaseNetPlus(nn.Module): # 1 U-net
    def __init__(
        self, 
        input_channels=3,
        encoder=[64, 128, 256, 512, 256], 
...
class WNet(nn.Module):
    def __init__(self):
        ...
        self.U_decoder = BaseNetPlus(input_channels=config.k, encoder=config.encoderLayerSizes,
